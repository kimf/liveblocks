...
5. From the command line, get the starter kit by running Create Liveblocks App.
   Follow through the instructions, selecting your project name, chosen
   authentication method, and install the app:
...
6. Enter the `.env.local` file, which is located alongside `package.json` at the
   root level in your project. This file will contain a randomly generated
   `NEXTAUTH_SECRET` that will be used for authentication, along with a number
   of empty variables for you to fill in.
...
After installing dependencies with `npm install`, the starter kit can be run
with
...
Auth0 authentication is now set up! Next,
[add yourself as a user](#how-to-sign-up) to test out your authentication.
